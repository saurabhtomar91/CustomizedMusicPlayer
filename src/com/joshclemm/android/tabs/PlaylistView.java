...
public class PlaylistView extends FrameLayout implements OnClickListener, OnSeekBarChangeListener, OnDrawerOpenListener, OnDrawerCloseListener, AdapterView.OnItemClickListener, DragListener, DropListener
{
...
private final Handler handler = new Handler() {
		...
		public void handleMessage(final Message msg)
		{
			switch (msg.what) {
				case ListItemGestureListener.MSG_FLING:
					...
					View listItem = listView.getChildAt(msg.arg1 - listView.getFirstVisiblePosition());

					Animation fling = AnimationUtils.loadAnimation(context, msg.arg2 < 0 ? R.anim.playlist_remove_left : R.anim.playlist_remove_right);
	...
	private void init()
	{
		...
		//playlist = new PlaylistRepository(context.getApplicationContext(), context.getContentResolver());
		...
		contractedNewsItemText = (TextView) findViewById(R.id.contracted_news_item_text);
		...
		//Intent intent = context.registerReceiver(changeReceiver, new IntentFilter(PlaybackService.SERVICE_CHANGE_NAME));
	/*	if (intent != null)
		{
		...
		intent = context.registerReceiver(updateReceiver, new IntentFilter(PlaybackService.SERVICE_UPDATE_NAME));
		if (intent != null)
		{
			updateReceiver.onReceive(context, intent);
		}
		closeReceiver = new PlaybackCloseReceiver();
		context.registerReceiver(closeReceiver, new IntentFilter(PlaybackService.SERVICE_CLOSE_NAME));
		errorReceiver = new PlaybackErrorReceiver();
		context.registerReceiver(errorReceiver, new IntentFilter(PlaybackService.SERVICE_ERROR_NAME));

		playlistChangedReceiver = new PlaylistChangedReceiver();
		context.registerReceiver(playlistChangedReceiver, new IntentFilter(PlaylistRepository.PLAYLIST_CHANGED));
*/
		...
		// Gesture detection
		gestureDetector = new GestureDetector(getContext(), new ListItemGestureListener(listView, handler));
	...
	{
		Cursor cursor = context.getContentResolver().query(PlaylistProvider.CONTENT_URI, null, null, null, PlaylistProvider.Items.PLAY_ORDER);
		return cursor;
	}*/
	...
	{
		playNow(Playable.PlayableFactory.fromPlaylistEntry(entry), PlaybackService.SERVICE_PLAY_ENTRY);
	}*/
	...
	/*private void configurePlayerControls()
	{
		...
		if (activeId != null)
		{
			...
			else
			{
				...
				moveControlsEnabled = true;
				rewindEnabled = false;
				// fastForwardEnabled = (playlist.getFirstUnreadEntry() !=
				// null);
			}
		}
		else
	...
	private class PlaybackChangeReceiver extends BroadcastReceiver
	{
		...
		public void onReceive(Context context, Intent intent)
		{
			try
			{
				Context serviceContext = context.createPackageContext(context.getPackageName(), Context.CONTEXT_INCLUDE_CODE | Context.CONTEXT_IGNORE_SECURITY);
...
/*	private class PlaybackUpdateReceiver extends BroadcastReceiver
	{
		...
		public void onReceive(Context context, Intent intent)
		{
			...
			Log.v(LOG_TAG, "Playback update; position = " + position + " millsecs; " + "downloaded = " + duration + " millsecs");
	 ...
	 * 
	 * @param position
	 *            The position of the item starting at 0
	...
	public PlaylistEntry getPlaylistEntryAtPosition(int position)
	{
		...
		if (c != null)
		{
			...
			if (entry == null)
			{
				//Log.e(LOG_TAG, "Couldn't find playlist item " + playlistId + " at position" + position);
			}
		}
		return entry;
	}
